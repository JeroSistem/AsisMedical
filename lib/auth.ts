...
export const authOptions: NextAuthOptions = {
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        try {
          ...
          // Para desarrollo, permitir login sin contraseña si el usuario existe
          // En producción, deberías verificar la contraseña hasheada
          const isPasswordValid = await bcrypt.compare(credentials.password, user.password || '') || 
                                 credentials.password === 'admin123'; // Fallback para desarrollo
  ...
  callbacks: {
    ...
    async redirect({ url, baseUrl }) {
      // Si la URL es la página de inicio, redirigir al dashboard

...
export default function InformeOportunidadPage() {
  ...
  const [centrosServicios, setCentrosServicios] = useState('');
  ...
  const handleGenerar = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: 'El listado de oportunidad se ha generado correctamente.',
      });
    } catch (error) {
      toast({
        title: 'Error',
  ...
  return (
    <ModulePageLayout
      title="GENERAR LISTADO OPORTUNIDAD"
      description="Genere el listado de oportunidad según los criterios seleccionados"
      actions={actions}
      maxWidth="7xl"
    >
        ...
        <CardContent>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="contratos" className="text-sm font-medium">
                  Contratos
                </Label>
                <div className="relative">
                  <select
              ...
              {/* Fila 2 */}
              <div className="space-y-2">
                <Label htmlFor="centros-servicios" className="text-sm font-medium">
                  Centros de servicios
                </Label>
                <Input
                  id="centros-servicios"
                  type="text"
                  value={centrosServicios}
                  onChange={(e) => setCentrosServicios(e.target.value)}
                  className="w-full"
                  placeholder="Ingrese centro de servicios"
                />
              </div>

              <div className="space-y-2">
                <Label htmlFor="centros-costos" className="text-sm font-medium">
                  Centros de costos
                </Label>
                <div className="relative">
                  <select
                    id="centros-costos"
                    value={centrosCostos}
                    onChange={(e) => setCentrosCostos(e.target.value)}
                    className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring appearance-none pr-8"
                  >
                    <option value="">Seleccione...</option>
                    <option value="costo1">Centro de Costo 1</option>
                    <option value="costo2">Centro de Costo 2</option>
                    <option value="costo3">Centro de Costo 3</option>
                  </select>
                  <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400 pointer-events-none" />
                </div>
              </div>
              ...
              <div className="space-y-2 md:col-span-2">
                <Label htmlFor="profesionales" className="text-sm font-medium">
                  Profesionales
                </Label>
                <div className="relative">
                  <select
